...
def CycleIR(images, reuse=False):
    with tf.variable_scope('CycleIR') as scope:
        ...
        final_max = tf.tile(tf.reduce_max(salmap, [1, 2], keep_dims=True),
                            [1, tf.shape(salmap)[1], tf.shape(salmap)[2]])
...
def model_test(sess, images, salmap, images_new, train_step, aspect_ratio, input_size, Sh, Sw):
    ResizeScale = 0.5
    # dirpath = './RetargetMeAll/' + str(ResizeScale) + '/'  # Vertical_
    dirpath = './RetargetMeAll/dataset-20110512/battleship'
    print(dirpath)
    img_ext = '.png'
    # outpath = './RetargetMeResult/' + str(ResizeScale) + '/'  # Vertical_
    outpath = './RetargetMeResult/'
    ...
    for i, (imgPath, imgName) in enumerate(zip(img_fnames, base_fnames)):
        ...
        imsave(outpath + 'step9299_sal_' + imgName[:-4] + '_BiGAN.png',
               imresize(salmap_[0, :, :], [shape[1], shape[2]], 'bicubic'))
        print('Sh_:', Sh_)
        print('Sw_:', Sw_)
